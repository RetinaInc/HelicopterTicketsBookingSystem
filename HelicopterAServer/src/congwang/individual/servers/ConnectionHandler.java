...
public class ConnectionHandler implements Runnable {
	...
	public String bookAticket(String s2, String s3, String s4) {
		...
		if (t.getLeftSeat() > 0) {
			...
			return "Sorry, There is no ticket available at this moment."
					+ "\n"
					+ "Notice:We do sale limited amount of tickets of other company"
					+ "\n" + "Please check other flights";
		}
	...
	public String cancelAticket(String s2, String s3) {
		...
		// c.setTicketNum(s2);
		// c.setPassport(s3);
		c = cs.loadByPassportAndTicketNum(s2, s3); // locate the related
													// customer infomation
		if (c == null) {
			return "Wrong input";
		}
		cs.deleteByPassportAndTicketNum(s2, s3); // delete customer infomation
	...
	public String addCustomer(String b2, String b3, String b4) {
		...
		int a = (int) (Math.random() * 1000) + (int) (Math.random() * 800); // generate
																			...
																			// ticket
																			// number
		String s = b2 + b3 + a; // generate the unique ticket number by
								// combining the flightNum, Date plust a random
								// number.
		c.setTicketNum(s);

		cs.add(c); // save customer information into Customerinfo database
