...
public class TimeTableService {
	...
	public void add(TimeTable t) {
		...
		String sql = "insert into t_timetable values (?,?,?,?,?,?,?)";
	...
	public List<TimeTable> listByDateAndDest(String s1, String s2) {
		Connection conn = DB.createConnection();
		String sql = "select * from t_timetable where leftDate = ? and destination = ?";
		PreparedStatement ps = DB.prepare(conn, sql);
		List<TimeTable> timetable = new ArrayList<>();
		try {
			ps.setString(1, s1);
	...
	public void deleteByFlightNum(String flightNum) {
		Connection conn = DB.createConnection();
		String sql = "delete from t_timetable where flightNum = ?";
		PreparedStatement ps = DB.prepare(conn, sql);
		try {
	...
	public void update(TimeTable t) {
		Connection conn = DB.createConnection();
		String sql = "update t_timetable set leftTime = ?,arriveTime = ?,destination = ?,company= ?"
	...
	public TimeTable loadById(String flightNum) {
		Connection conn = DB.createConnection();
		String sql = "select * from t_timetable where flightNum = ?";
		PreparedStatement ps = DB.prepare(conn, sql);
		TimeTable t = null;
		try {
			ps.setString(1, flightNum);
	...
	public TimeTable loadByFlightNoAndDate(String flightNum, String leftDate) {
		Connection conn = DB.createConnection();
		String sql = "select * from t_timetable where flightNum = ? and leftDate = ?";
		PreparedStatement ps = DB.prepare(conn, sql);
		TimeTable t = null;
		try {
			ps.setString(1, flightNum);
